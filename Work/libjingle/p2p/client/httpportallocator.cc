...
HttpPortAllocator::HttpPortAllocator(talk_base::NetworkManager* network_manager, const std::string &user_agent,
            const char * szStunAddr /*= 0*/, unsigned short wStunPort /*= 0*/, const char * szRelayAddr /*= 0*/ )
  ...
  //stun_hosts_.push_back(talk_base::SocketAddress("stun.l.google.com",19302));
      if ( szRelayAddr )
      {
        relay_hosts_.push_back(szRelayAddr);
      }
      else
      {
        relay_hosts_.push_back("0.0.0.0");
      }

      if ( szStunAddr )
      {
        stun_hosts_.push_back(talk_base::SocketAddress(szStunAddr,wStunPort));
      }
      else
      {
        stun_hosts_.push_back(talk_base::SocketAddress("0.0.0.0",3478));
      }
