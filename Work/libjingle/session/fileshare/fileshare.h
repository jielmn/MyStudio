...
public:
  ...
  FileShareSession(cricket::Session* session, const std::string &user_agent, unsigned long dwShareId = 0, const char * szLocalDir = 0);
  ...
  // added by shijie
  unsigned long m_dwShareId;
  unsigned long m_dwCancelFlag;
  std::string   m_sCompletePath;
  std::string   m_sSaveAsName;
  int  err(){ return err_;}
  // end of adding
private:
...
class FileShareSessionClient :  public SessionClient
{
 public:
  ...
  FileShareSession *CreateFileShareSession( unsigned long dwShareId = 0, const char * szLocalDir = 0 );
