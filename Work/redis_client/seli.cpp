...
// select多路复用
// nMs单位， 毫秒 
int  seliSelect( DWORD * pdwMs, FdType fdCared /*= -1*/ )
{
    struct timeval  tv;
    ...
    for ( nIndex = 0; nIndex < s_max_fd_cnt; nIndex++ )
    {
        if (    s_nodes[nIndex].valid && !s_nodes[nIndex].bException 
             && ( (INVALID_SOCKET == fdCared && s_nodes[nIndex].bAsyn) || nIndex == fdCared ) )
        {
            // 防止回调函数中用li系列函数造成混淆
...
/**************************************** li *******************************************************/
  ...
  FdType liOpen ( liProtocol_t protocol, DWORD ipAddr, WORD wPort, LPLIEVENTHANDLER eventHandler, void* context,
                              BOOL bTcpKeepAlive, BOOL bAsyn /*= TRUE*/ )
{
    int s,nIndex;
