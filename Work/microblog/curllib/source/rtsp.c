...
CURLcode Curl_rtsp(struct connectdata *conn, bool *done)
{
  ...
  *done = TRUE;
  ...
  if(!data->state.proto.rtsp) {
    ...
    rtsp = (struct RTSP *)calloc(1, sizeof(struct RTSP));
    if(!rtsp)
  ...
  if(rtspreq == RTSPREQ_ANNOUNCE ||
    ...
    if(data->set.upload) {
      ...
      postsize = (data->set.postfieldsize != -1)?
        ...
        (data->set.postfields? (curl_off_t)strlen((const char *)data->set.postfields):0);
...
CURLcode Curl_rtsp_rtp_readwrite(struct SessionHandle *data,
  ...
  if(rtspc->rtp_buf) {
    ...
    char *newptr = (char *)realloc(rtspc->rtp_buf, rtspc->rtp_bufsize + *nread);
    if(!newptr) {
