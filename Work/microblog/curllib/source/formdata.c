            ...
            struct curl_httppost **last_post)
{
  ...
  post = (struct curl_httppost *)calloc(1, sizeof(struct curl_httppost));
  if(post) {
    post->name = name;
...
{
  FormInfo *form_info;
  form_info = (FormInfo *)calloc(1, sizeof(struct FormInfo));
  if(form_info) {
    if(value)
...
static CURLcode AddFormData(struct FormData **formp,
                            ...
                            curl_off_t *size)
{
  struct FormData *newform = (struct FormData *)malloc(sizeof(struct FormData));
  if(!newform)
