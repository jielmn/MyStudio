...
static struct Curl_sh_entry *sh_addentry(struct curl_hash *sh,
                                         ...
                                         struct SessionHandle *data)
{
  struct Curl_sh_entry *there = (struct Curl_sh_entry *)Curl_hash_pick(sh, (char *)&s, sizeof(curl_socket_t));
                                ...
                                CURL *easy_handle)
{
  ...
  easy = (struct Curl_one_easy *)calloc(1, sizeof(struct Curl_one_easy));
  if(!easy)
  ...
  /* set the easy handle */
  easy->easy_handle = (struct SessionHandle *)easy_handle;
  ...
  Curl_easy_addmulti((struct SessionHandle *)easy_handle, multi_handle);
  ...
  if((multi->num_easy * 4) > multi->connc->num) {
    ...
    if(newmax > multi->connc->num) {
      ...
      CURLcode res = Curl_ch_connc((struct SessionHandle *)easy_handle, multi->connc, newmax);
      if(res != CURLE_OK) {
    ...
    case CURLM_STATE_PERFORM:
      ...
      if(easy->result) {
        ...
        if(easy->easy_conn->recv_pipe->head)
          Curl_expire((struct SessionHandle *)easy->easy_conn->recv_pipe->head->ptr, 1);
...
CURLMcode curl_multi_perform(CURLM *multi_handle, int *running_handles)
{
  ...
  do {
    ...
    if(t) {
      struct SessionHandle *d = (struct SessionHandle *)t->payload;
                         ...
                         struct Curl_one_easy *easy)
{
  ...
  for(i=0; (i< MAX_SOCKSPEREASYHANDLE) &&
    ...
    entry = (struct Curl_sh_entry *)Curl_hash_pick(multi->sockhash, (char *)&s, sizeof(s));
          ...
          if (easy_conn->recv_pipe && easy_conn->recv_pipe->size > 1) {
            ...
            if (entry->easy == easy->easy_handle) {
              if (isHandleAtHead(easy->easy_handle, easy_conn->recv_pipe))
                entry->easy = (struct SessionHandle *)easy_conn->recv_pipe->head->next->ptr;
              else
                entry->easy = (struct SessionHandle *)easy_conn->recv_pipe->head->ptr;
            }
          }
          if (easy_conn->send_pipe  && easy_conn->send_pipe->size > 1) {
            ...
            if (entry->easy == easy->easy_handle) {
              if (isHandleAtHead(easy->easy_handle, easy_conn->send_pipe))
                entry->easy = (struct SessionHandle *)easy_conn->send_pipe->head->next->ptr;
              else
                entry->easy = (struct SessionHandle *)easy_conn->send_pipe->head->ptr;
            }
          }
                              ...
                              int *running_handles)
{
  ...
  if(checkall) {
    ...
    struct Curl_sh_entry *entry =
      (struct Curl_sh_entry *)Curl_hash_pick(multi->sockhash, (char *)&s, sizeof(s));
    ...
    if(!entry)
      ...
      if(data->set.one_easy->easy_conn) {
        if ((ev_bitmask & CURL_POLL_OUT) &&
          ...
          data = (struct SessionHandle *)data->set.one_easy->easy_conn->send_pipe->head->ptr;
        else if ((ev_bitmask & CURL_POLL_IN) &&
                 data->set.one_easy->easy_conn->recv_pipe &&
                 data->set.one_easy->easy_conn->recv_pipe->head)
          data = (struct SessionHandle *)data->set.one_easy->easy_conn->recv_pipe->head->ptr;
      }
  ...
  do {
    ...
    if(t) {
      /* assign 'data' to be the easy handle we just removed from the splay
         tree */
      data = (struct SessionHandle *)t->payload;
                                              ...
                                              struct connectdata *conn)
{
  ...
  if (pipeline == conn->send_pipe && sendhead != conn->send_pipe->head) {
      ...
      Curl_expire((struct SessionHandle *)conn->send_pipe->head->ptr, 1);
  }
...
static int checkPendPipeline(struct connectdata *conn)
{
  ...
  if (conn->server_supports_pipelining || pipeLen == 0) {
    ...
    while(pipeLen < maxPipeLen && curr) {
      ...
      Curl_pgrsTime((struct SessionHandle *)curr->ptr, TIMER_PRETRANSFER);
                            ...
                            curl_socket_t s, void *hashp)
{
  ...
  if(s != CURL_SOCKET_BAD)
    there = (struct Curl_sh_entry *)Curl_hash_pick(multi->sockhash, (char *)&s, sizeof(curl_socket_t));
                             ...
                             struct SessionHandle *data)
{
  ...
  if (add) {
    cl = (struct closure *)calloc(1, sizeof(struct closure));
    if(!cl)
      return CURLM_OUT_OF_MEMORY;
