...
// 2012-02-02 revised by sunfei C++中枚举值不能进行自增运算
//       http->sending++; /* move one step up */
	  ...
	  case HTTPSEND_NADA:
	  	...
	  	break;
	  case HTTPSEND_REQUEST:
		  http->sending = HTTPSEND_BODY;
		  break;
	  case HTTPSEND_BODY:
		  http->sending = HTTPSEND_LAST;
		  break;
	  }
// revised end	  
...
{
  return (Curl_send_buffer *)calloc(1, sizeof(Curl_send_buffer));
}
                        ...
                        CURLcode status, bool premature)
{
  ...
  if(http->send_buffer) {
    Curl_send_buffer *buff = (Curl_send_buffer *)http->send_buffer;
  ...
  if(conn->bits.httpproxy && !conn->bits.tunnel_proxy)  {
    ...
    if(conn->host.dispname != conn->host.name) {
      ...
      if(ptr) {
        ...
        newurl = (char *)malloc(urllen + newlen - currlen + 1);
        if(newurl) {
  ...
  case HTTPREQ_POST:
    ...
    if(conn->bits.authneg)
      ...
      postsize = (data->set.postfieldsize != -1)?
        ...
        (data->set.postfields? (curl_off_t)strlen((const char *)data->set.postfields):-1);
    }
    if(!data->req.upload_chunky) {
                                       ...
                                       bool *stop_reading)
{
  ...
  do {
    ...
    k->end_ptr = (char *)memchr(k->str_start, 0x0a, *nread);
