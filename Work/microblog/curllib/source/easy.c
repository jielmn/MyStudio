...
void Curl_easy_addmulti(struct SessionHandle *data,
                        void *multi)
{
  data->multi = (struct Curl_multi *)multi;
  if(multi == NULL)
...
CURL *curl_easy_duphandle(CURL *incurl)
{
  ...
  struct SessionHandle *outcurl = (struct SessionHandle *)calloc(1, sizeof(struct SessionHandle));
    ...
    do {
      ...
      if(data->state.tempwrite && (tempsize - chunklen)) {
        ...
        */
        ...
        newptr = (char *)realloc(data->state.tempwrite, tempsize);
...
CURLcode curl_easy_recv(CURL *curl, void *buffer, size_t buflen, size_t *n)
{
  ...
  *n = 0;
  ret = Curl_read(c, sfd, (char *)buffer, buflen, &n1);
