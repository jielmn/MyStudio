                                ...
                                long amount) /* set -1 to use default */
{
  ...
  c = (struct conncache *)calloc(1, sizeof(struct conncache));
  if(!c)
    return NULL;

  c->connects = (struct connectdata**)calloc((size_t)default_amount, sizeof(struct connectdata *));
  if(!c->connects) {
    free(c);
    return NULL;
  }
                       ...
                       long newamount)
{
  ...
  if(newamount > 0) {
    ...
    newptr = (struct connectdata**)realloc(c->connects, sizeof(struct connectdata *) * newamount);
    if(!newptr)
    ...
    if(!argptr || data->set.postfieldsize == -1)
      ...
      if((data->set.postfieldsize < 0) ||
        ...
        /* Allocate even when size == 0. This satisfies the need of possible
           ...
           form data.
        */
        p = (char *)malloc((size_t)(data->set.postfieldsize?
                            ...
                            bool *async)
{
  ...
  *in_connect = conn;
  ...
  if(data->multi && Curl_multi_canPipeline(data->multi) &&
    ...
    conn->master_buffer = (char *)calloc(BUFSIZE, sizeof (char));
