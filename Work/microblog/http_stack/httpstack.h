...
public:
                   ...
                   EFormatDataType eType =  FORMAT_DATA_TYPE_NORMAL, BOOL bUtf8 = FALSE )
    {
        ...
        if ( FORMAT_DATA_TYPE_UPLOAD == m_eType )
        {
            ...
            {
                char * pTmp = 0;
                lo_C2Utf8( &pTmp, m_sFileName.c_str() );
                m_sFileName = pTmp;
                free( pTmp );
            }
        }
...
typedef WeiboCallback HttpCallback;
...
public:
    ...
    CHttpRequest( const std::string & sUrl = "", EHttpMethod eMethod = HTTP_METHOD_GET, const std::string & sData = "", 
                  EContentType eContentType = HTTP_CONTENT_TYPE_NORMAL, HttpCallback pfnCallBack = 0 )
    {
        m_sUrl = sUrl;
    ...
    void SetMethod( EHttpMethod method ){ m_eMethod = method; }
    ...
    void AppendOption( const std::string & sKey, const std::string & sValue )
    {
        CHttpOption * pOption = new CHttpOption;
        pOption->m_sKey   = sKey;
        pOption->m_sValue = sValue;
    ...
    void AppendFormatData( const std::string & sLine,  CHttpFormData::EFormatDataType eType =  CHttpFormData::FORMAT_DATA_TYPE_NORMAL )
    {
        ...
        char buf[max_size] = { 0 };
        GetStringFromUrl( buf, max_size, sValue.c_str() );
...
const DWORD HTTP_CONNECTION_TIMEOUT   = 20;       //单位:秒
const DWORD HTTP_RECEIVE_BUFFER_SIZE  = 8192;
#ifdef _DEBUG
const DWORD HTTP_IDLE_TIMEOUT         = 60;     //单位:秒
#else
#if defined (_IOS_PLATFORM_)
const DWORD HTTP_IDLE_TIMEOUT         = 10;    //单位:秒
#else
const DWORD HTTP_IDLE_TIMEOUT         = 60;    //单位:秒
#endif
#endif

#define  MAX_DNS_DOMAIN_NAME_LEN      64
