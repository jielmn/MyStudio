...
HttpCallback  GetCallback( const CHttpRequest & request )
{
    if ( request.m_callback )
    {
        return request.m_callback;
    }
    else
    {
        return s_http_callback;
    }
}
...
void  CHttpTransfer::Reconnect()
{
    ...
    if ( !m_bUseSock5 )
    {
        ...
        if ( m_fd == INVALID_SOCKET )
        {
            for ( it = m_vTransactions.begin(); it != m_vTransactions.end(); it++ )
            {
                ...
                HttpCallback pfnCallBacl = GetCallback( pTransaction->m_request );

                if ( pfnCallBacl )
                {
                    pfnCallBacl( 0, pTransaction->m_response.GetHttpState(), 
    ...
    else
    {
        ...
        if ( m_fd == INVALID_SOCKET )
        {
            ...
            {
                CHttpTransaction * pTransaction = *it;
                HttpCallback pfnCallBack = GetCallback( pTransaction->m_request );

                if ( pfnCallBack )
                {
                    pfnCallBack( 0, pTransaction->m_response.GetHttpState(), 
                                pTransaction->m_response.GetData(), pTransaction->m_response.GetDataSize(), 
                                (char *)pTransaction->m_TotalHeader.GetData(),  pTransaction->m_TotalHeader.GetDataLength(),
                                pTransaction->m_pContext );
                }
...
void  CHttpTransfer::OnEventClose( int nErr /*= 0*/ )
{
    ...
    if (  0 != nErr ||  !( TRANSFER_STATE_OK == m_eState || TRANSFER_STATE_WAITING_SERVER_CLOSE == m_eState )  )
    {
        std::vector<CHttpTransaction*>::iterator it;
        for ( it = m_vTransactions.begin(); it != m_vTransactions.end(); it++ )
        {
            ...
            if ( pfnCallBack )
            {
                pfnCallBack( nErr, pTransaction->m_response.GetHttpState(), 
                            pTransaction->m_response.GetData(), pTransaction->m_response.GetDataSize(), 
                            (char *)pTransaction->m_TotalHeader.GetData(),  pTransaction->m_TotalHeader.GetDataLength(),
                            pTransaction->m_pContext );
            }
    ...
    while ( len > 0 )
    {
        ...
        if ( CHttpResponse::PROGRESS_OK == pResponse->GetProgressState() )
        {
            ...
            if ( !pResponse->HasLocation() )
            {
                ...
                {
                    // 通知上层
                    pfnCallBack( 0, pResponse->GetHttpState(), pResponse->GetData(), pResponse->GetDataSize(), 
                        (char *)pTransaction->m_TotalHeader.GetData(),  pTransaction->m_TotalHeader.GetDataLength(),
                        pTransaction->m_pContext );
                    // END 通知上层
                }
...
#if   SHOW_HTTP_DATA
    sDebugInfo = "Send request\r\n" + sDebugInfo + "\r\n\r\n";
    MICROBLOG_LOG( "%s", sDebugInfo.c_str() );
#endif
...
static BOOL  s_bHttpInited = FALSE;
