...
{
	ifstream fs;
	fs.open( filename.c_str(), ios_base::in | ios_base::binary );
...
string  MyConfigFile::operator[] ( const char * key ) const
{
    std::string k = key;
    return this->read( k );
}
...
void MyConfigFile::WriteItem2File( ofstream & fs, PTMyCfgItem pItem, 
                             ...
                             unsigned int nMinKeyDataWidth /* = 40 */  )
{
	...
	if ( 0 == pvItems )
	{
        ...
        {
            WriteItem2File( fs, *y, nMinKeyWidth, nMinKeyDataWidth );
        }
	}
	else
	{
		...
		for ( q = pvItems->begin(); q != pvItems->end(); q++ )
		{
            ...
            WriteItem2File( fs, &item, nMinKeyWidth, nMinKeyDataWidth );
		}
	}
